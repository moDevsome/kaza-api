...
class EquipmentRepository extends ServiceEntityRepository
{
    ...
    /**
     * Find Equipment entities by making a lookup on the name
     * Return the full DB entity
     * @param string $queryString
     * @param ?string $operator
     * @return Equipment[] Returns an array of equipment entity
     *
    ...
    public function findByName(string $queryString, ?string $operator = '='): array
    {
        ...
        $rsm->addEntityResult('Api\Entity\Equipment', 'l');
        $rsm->addFieldResult('l', 'id', 'id');
        $rsm->addFieldResult('l', 'name', 'name');

        $sqlQuery = 'select l.id, l.name from equipment l where l.name ' . $operator . ' :value';
